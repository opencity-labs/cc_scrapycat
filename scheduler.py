...
def setup_scrapycat_schedule(
    cheshire_cat, settings: Optional[Dict[str, Any]] = None
) -> None:
    ...
    try:
        ...
        log.info(
            f"Scheduled ScrapyCat command '{scheduled_command}' to run daily at {schedule_hour:02d}:{schedule_minute:02d} UTC"
        )
        ...
        try:
            ...
            if not scheduler_running:
                log.warning(
                    "White Rabbit scheduler is not running! This may be why jobs don't execute."
                )
            ...
            if job:
                ...
                if "next_run" in job:
                    ...
                    if time_diff.total_seconds() < 0:
                        log.warning(
                            "Job is scheduled in the past! This might be why it's not executing."
                        )
                else:
...
def save_plugin_settings_to_file(
